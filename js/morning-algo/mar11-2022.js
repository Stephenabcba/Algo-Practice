...
function mergeSort(arr) {
    // base case: array with 0 or 1 item, which is by default sorted
    if (arr.length <= 1) {
        return arr
    }
    const mid = Math.floor(arr.length / 2) // midpoint to split the array, exact location of mid is not that important
    // recursively merge sort left side and right side
    // both left and right sides will be sorted arrays
    // and then use the helper function mergeSortedArrays() to combine left and right
    return mergeSortedArrays(mergeSort(arr.slice(0, mid)), mergeSort(arr.slice(mid, arr.length)))
}
...
function mergeSortedArrays(arr1, arr2) {
    ...
    while (leftIdx < arr1.length && rightIdx < arr2.length) {
        if (arr1[leftIdx] < arr2[rightIdx]) {
            returnArr.push(arr1[leftIdx])
            leftIdx++
        } else {
            returnArr.push(arr2[rightIdx])
            rightIdx++
        }
    }
