...
var fourSumCount = function(nums1, nums2, nums3, nums4) {
    ...
    //             if (threeSum.hasOwnProperty(nums1[i] + nums2[j] + nums3[k])){
    //                 threeSum[(nums1[i] + nums2[j] + nums3[k])] ++
    //             } else {
    //                 threeSum[(nums1[i] + nums2[j] + nums3[k])] = 1
    //             }
    //         }
    //     }
    // }
    // for(let l = 0; l < numsLength; l++) {
    //     if (threeSum.hasOwnProperty(nums4[l] * -1)) sumCount += threeSum[(nums4[l] * -1)]
    // }
    let twoSum = {}
    for (let i = 0; i < numsLength; i++) {
        for (let j = 0; j < numsLength; j++) {
            if (twoSum.hasOwnProperty(nums1[i] + nums2[j])){
    ...
    for(let k = 0; k < numsLength; k++) {
        for(let l = 0; l < numsLength; l++) {
            if (twoSum.hasOwnProperty((nums3[k] + nums4[l]) * -1)) sumCount += twoSum[((nums3[k] + nums4[l]) * -1)]
        }
    }
    return sumCount
};
...
console.log(fourSumCount(two_nums1,two_nums2,two_nums3,two_nums4));